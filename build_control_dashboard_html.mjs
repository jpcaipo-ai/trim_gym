  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .efficiency-panel {
      margin-top: 14px;
      display: grid;
      grid-template-columns: minmax(170px, .8fr) repeat(5, minmax(120px, 1fr));
      gap: 10px;
      align-items: stretch;
    }
    .efficiency-card {
    ...
    @media (max-width: 1160px) {
      ...
      .efficiency-panel { grid-template-columns: repeat(3, minmax(140px, 1fr)); }
    }
    @media (max-width: 720px) {
      ...
      .efficiency-panel { grid-template-columns: 1fr; }
...
<body>
  ...
  <script>
    ...
    const investmentByMonth = {
      '2025-10': { ads: 2778.09, service: 5950, note: 'Oct-dic repartido proporcionalmente.' },
      '2025-11': { ads: 2778.09, service: 5950, note: 'Oct-dic repartido proporcionalmente.' },
      '2025-12': { ads: 2778.10, service: 5950, note: 'Oct-dic repartido proporcionalmente.' },
      '2026-01': { ads: 3200, service: 4300, note: 'Ene-may repartido proporcionalmente.' },
      '2026-02': { ads: 3200, service: 4300, note: 'Ene-may repartido proporcionalmente.' },
      '2026-03': { ads: 3200, service: 4300, note: 'Ene-may repartido proporcionalmente.' },
      '2026-04': { ads: 3200, service: 4300, note: 'Ene-may repartido proporcionalmente.' },
      '2026-05': { ads: 3200, service: 4300, note: 'Ene-may repartido proporcionalmente.' },
      '2026-06': { ads: 3923.69, service: 4318, note: 'Dato informado al 30/06 10:37 pm.' },
      '2026-08': { ads: 3890, service: 4318, note: 'Pauta estimada desde CP etapa del pipeline.' }
    };
    const efficiencyMonths = [...new Set([...ventas.map(r => r.Mes), ...Object.keys(investmentByMonth)].filter(Boolean))].sort();
    let efficiencyMonth = efficiencyMonths[efficiencyMonths.length - 1] || '';
    const lists = {
      ...
      months.forEach((row, i) => {
        ...
        if (baseH > 0) {
          ...
          if (baseH > 24) {
            ...
            ctx.textAlign = 'center';
            ctx.fillText('Sin Llama', x + bw / 2, y + baseH / 2 - 4);
    ...
    function metricPurchasesForMonth(mes) {
      const q = norm(els.buscar.value);
      return (METRICS.Compras || []).map(r => ({
        ...r,
        Mes: monthFromDateText(r.Inscripcion),
    ...
    function impactValueForMonth(mes, includeLtv) {
      const acquisition = acquisitionMonthlyTotals(true)
        .filter(r => r.key === mes)
        .reduce((a, r) => a + Number(r.value || 0), 0);
      if (!includeLtv) return acquisition;
      const recurring = metricPurchasesForMonth(mes)
        .filter(r => norm(r['Tipo plan']).includes('renov') && !norm(r['Tipo servicio']).includes('matr'))
        .reduce((a, r) => a + Number(r.Pago || 0), 0);
      return acquisition + recurring;
    }
    function renderEfficiencyPanel() {
      const monthOptionsHtml = efficiencyMonths.map(m => '<option value="' + escapeHtml(m) + '"' + (m === efficiencyMonth ? ' selected' : '') + '>' + escapeHtml(captureMonthLabel(m) || m) + '</option>').join('');
      const investment = investmentByMonth[efficiencyMonth] || { ads: 0, service: 0, note: 'Sin pauta cargada para este mes.' };
      const ads = Number(investment.ads || 0);
      const service = Number(investment.service || 0);
      const totalInvestment = ads + service;
      const monthRows = ventas.filter(r => r.Mes === efficiencyMonth && (els.sede.value === 'Todos' || r.Sede === els.sede.value));
      const totalBilling = monthRows.reduce((a, r) => a + Number(r.Pago || 0), 0);
      const impact = impactValueForMonth(efficiencyMonth, impactMode === 'ltv');
      const roas = ads ? impact / ads : null;
      const mer = ads ? totalBilling / ads : null;
      document.querySelector('#efficiencyPanel').innerHTML =
      ...
      document.querySelector('#efficiencyMonth')?.addEventListener('input', event => {
        efficiencyMonth = event.target.value;
        update();
      });
    }
    function firstPurchaseType(value) {
      const amount = Number(value || 0);
      if (amount >= 1950) return 'pt';
      if (amount >= 1800) return 'intro';
      return 'semi';
    }
    function firstPurchaseTier(value) {
      const amount = Number(value || 0);
      if (amount >= 60000) return { color: '#b7791f', label: 'S/ 60k+', name: 'Mes estrella' };
      if (amount >= 40000) return { color: '#b94a48', label: 'S/ 40k+', name: 'Alto impacto' };
    ...
    function firstPurchaseTypeMonthlyTotals(acqTotals) {
      const raw = group(acquisitionRows(false), r => r.Sede + '|' + r.Mes, (r, key) => ({
        ...
        const amount = Number(r.Base || 0);
        const type = firstPurchaseType(amount);
      ...
      const bySedeMonth = group(acqTotals, r => r.sede + '|' + r.key, (r, key) => ({
        ...
        const rawRow = rawMap.get(r.sede + '|' + r.key) || { intro: 0, pt: 0, semi: 0 };
        const rawTotal = rawRow.intro + rawRow.pt + rawRow.semi;
        const nuevosTotal = Number(r.nuevos || 0);
        const scale = rawTotal ? nuevosTotal / rawTotal : 1;
        s.matricula += Number(r.matricula || 0);
        s.ajusteIntro += Number(r.ajusteIntro || 0);
        s.intro += rawTotal ? rawRow.intro * scale : nuevosTotal;
      ...
      return byMonth.map(r => ({
        ...
        pt: r.pt,
        semi: r.semi,
        total: r.matricula + r.ajusteIntro + r.intro + r.pt + r.semi
      })).sort((a, b) => a.key.localeCompare(b.key));
    }
    function attributedComboChart(id, rows) {
      ...
      const matriculas = displayMonths.map(r => tierMode ? 0 : r.matricula);
      const ajustesIntro = displayMonths.map(r => tierMode ? 0 : r.ajusteIntro);
      const pts = displayMonths.map(r => tierMode ? 0 : r.pt);
      const semis = displayMonths.map(r => tierMode ? 0 : r.semi);
      const recurrentesPrev = displayMonths.map(r => monthOnly ? 0 : r.recurrentePrev);
      const recurrentesSame = displayMonths.map(r => monthOnly ? 0 : r.recurrenteSame);
      const { ctx, w, h } = canvasCtx(id);
      ctx.clearRect(0, 0, w, h);
      const pad = { l: tierMode ? 70 : 58, r: 24, t: 62, b: 64 };
      ...
      labels.forEach((label, i) => {
        ...
        top -= semiH; ctx.fillRect(x, top, bw, semiH);
      ...
      if (detailMode) {
        ...
        ctx.fillStyle = '#c58a00'; ctx.fillRect(pad.l + 120, 12, 11, 11);
        ctx.fillStyle = '#334155'; ctx.fillText('Matrícula', pad.l + 136, 22);
        ctx.fillStyle = '#e0aa3e'; ctx.fillRect(pad.l + 250, 12, 11, 11);
        ctx.fillStyle = '#334155'; ctx.fillText('Ajuste intro', pad.l + 266, 22);
        ctx.fillStyle = '#2468d8'; ctx.fillRect(pad.l + 400, 12, 11, 11);
        ctx.fillStyle = '#334155'; ctx.fillText('PT', pad.l + 416, 22);
        ctx.fillStyle = '#0f8b83'; ctx.fillRect(pad.l + 470, 12, 11, 11);
        ctx.fillStyle = '#334155'; ctx.fillText('Semi', pad.l + 486, 22);
      } else if (tierMode) {
        const tiers = [
          ['#c58a00', 'S/ 15k+'],
          ['#1f8a4c', 'S/ 30k+'],
      ...
      if (!monthOnly && !detailMode) {
        ctx.fillStyle = '#c58a00'; ctx.fillRect(pad.l + 135, 12, 11, 11);
        ctx.fillStyle = '#334155'; ctx.fillText('Matrícula', pad.l + 151, 22);
        ctx.fillStyle = '#e0aa3e'; ctx.fillRect(pad.l + 270, 12, 11, 11);
        ctx.fillStyle = '#334155'; ctx.fillText('Ajuste intro', pad.l + 286, 22);
        ctx.fillStyle = '#c44545'; ctx.fillRect(pad.l + 430, 12, 11, 11);
        ctx.fillStyle = '#334155'; ctx.fillText('Recompra', pad.l + 446, 22);
      }
      if (!monthOnly && detailMode) {
        ctx.fillStyle = '#c44545'; ctx.fillRect(pad.l + 560, 12, 11, 11);
        ctx.fillStyle = '#334155'; ctx.fillText('Recompra', pad.l + 576, 22);
      }
    }
    ...
    document.querySelectorAll('[data-impact-mode]').forEach(button => {
      button.addEventListener('click', () => {
        impactMode = button.dataset.impactMode || 'ltv';
        document.querySelectorAll('[data-impact-mode]').forEach(tab => {
